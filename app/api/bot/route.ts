...
2. Телеграм бот дотор <b>Холболт</b> -> <b>Түлхүүр үүсгэх</b> гэснийг сонгоно уу.
...
Дэлгэрэнгүй зааврууд:
https://www.galvpn.com/instructions
`,
            { parse_mode: "HTML" }
        );
    })
    .row()
    .text("Төлбөр төлөх 📲💳", async (ctx) => {
        const cnt = `Хэрэв танд энэ үйлчилгээ таалагдаж байгаа бөгөөд та үүнийг үргэлжлүүлэн ашиглахыг хүсч байгаа бол сард <b>${
            config.paymentAmountPerMonth
        }</b> рубль төлнө үү.

Гүйлгээний утга хэсэгт: <code>${`${ctx.from.id}${tgDomain}`}</code>
Мөнгө хийх данс: <code>${config.accountDetails}</code> (${config.bankType})
        `;
        await ctx.editMessageText(cnt, { parse_mode: "HTML" });
    });
...
Та дараах товчлууруудаар дамжуулан боттой харилцаж болно. Хэрэв та шинээр эхлүүлэхийг хүсвэл /start ажиллуулна уу. ${
